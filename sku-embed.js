...
const authenticatedRequest = rawRequest.defaults({
  baseUrl: `https://api.3dplatforma.ru/api`,
  headers: {
...
function generateEmbedCode(SKUorBarcode) {
    ...
    body: {
      data: {
        ...
        attributes: {
          ...
          logo: true, // whether to display 3D platform logo or not
  ...
  .then((body) => {
    ...
    //   src="https://api.3dplatforma.ru/api/player/<model-id>/embedded?autorun=1&closebutton=1&hidecontrols=0&logo=1&hidefullscreen=0" >
    // </iframe>
    return body.data;
  });
}
